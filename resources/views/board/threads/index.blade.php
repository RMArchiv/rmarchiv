                    ...
                    <ul class="list-group">
                        @foreach($threads as $thread)
                            <li class="list-group-item media" style="margin-top: 0px;">
                                <a class="float-end" href="{{ route('board.thread.show', $thread->id) }}"><span class="badge">{{ $thread->posts->count() }}</span></a>
                                <a class="float-start" href="{{ url('users', $thread->user->id) }}"><img class="me-3" width="42px" src="//{{ config('app.avatar_path') }}?size=42&gender=male&id={{ $thread->user->id }}" alt="{{ $thread->user->name }}"></a>
                                <div class="thread-info">
                                    ...
                                    <div class="media-body" style="font-size: 12px;">
                                        ...
                                        <a href='{{ url('users', $thread->last_user->id) }}' class='usera' title="{{ $thread->last_user->name }}">
                                            <img width="16px" class="img-rounded" src='//{{ config('app.avatar_path') }}?size=16&gender=male&id={{ $thread->last_user->id }}' alt="{{ $thread->last_user->name }}"/>
